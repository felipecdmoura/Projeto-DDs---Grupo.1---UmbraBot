module.exports = {
    ...
    execute(BOT, message, ARGS, DISCORD) {
        ...
        let region = {// identifico a possivel região do servidor
        "brazil": "Brazil",
        ...
        let offline = member.cache.filter(m => m.user.presence.status === "offline").size,//quantidade de membros offline
            online = member.cache.filter(m => m.user.presence.status === "online").size,// quantidade de membros online
