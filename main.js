...
BOT.on(`message`, (message) => {
  ...
  switch (COMMAND) {
    ...
    case `addrolelist`:
      ...
      if (ARGS.length === 0)
        return message.channel.send(
          "Digite --addRole <nome do cargo>. (Limite 5 cargos.)"
        );
    ...
    case `removerolelist`:
      ...
      if (ARGS.length === 0)
        return message.channel.send("Digite --removerolelist <nome do cargo>.");
    ...
    case "putrole":
      ...
      if (target1) {
        ...
        if (ARGS[1] === 0)
          return message.channel.send(
            "Digite --putRole <@nome da pessoa> <nome do cargo>."
          );
        ...
        if (cargo1 == ARGS[1]) {
          ...
          message.channel.send(
            `O usuário ${memberTarget1} foi adicionado ao cargo ${ARGS[1]}.`
          );
    ...
    case "removerole":
      ...
      if (target) {
        ...
        if (cargo1 == ARGS[1]) {
          ...
          message.channel.send(
            `O usuário ${memberTarget} foi removido do cargo ${ARGS[1]}.`
          );
        } else if (cargo2 == ARGS[1]) {
          memberTarget.roles.remove(Role.id);
          message.channel.send(
